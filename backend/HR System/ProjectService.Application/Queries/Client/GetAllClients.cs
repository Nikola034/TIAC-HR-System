...
namespace Application.Queries.Client
{
    public class GetAllClientsHandler : IRequestHandler<GetAllClientsQuery, GetAllClientsQueryResponse>
    {
        ...
        {
            var clients = await _clientRepository.GetAllClientsAsync(req.PageNumber,req.ItemNumber,req.Name,req.Country, ct);
            ...
            var totalPages = await _clientRepository.GetTotalPageNumber(req.ItemNumber,req.Name,req.Country,ct);
            return new GetAllClientsQueryResponse(list,req.PageNumber,req.ItemNumber,totalPages);
        }
        
    }

    public record GetAllClientsQuery(int PageNumber, int ItemNumber, string Name, string Country) : IRequest<GetAllClientsQueryResponse>;
