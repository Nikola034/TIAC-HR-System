...
namespace EmployeeService.Application.Common.Repositories
{
    public interface IHolidayRequestRepository
    {
        ...
        public Task<IEnumerable<Core.Entities.HolidayRequest>> GetAllHolidayRequestsBySenderIdAsync(Guid senderId, int page, int items, string status, CancellationToken cancellationToken = default);
